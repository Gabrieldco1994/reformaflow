...
16. **Regra de categoria (`MerchantCategory`) só muda CATEGORIA, nunca valor/caixa.** Auto-aplicação no ingest só para regra manual confirmada; PIX PF nunca auto-aplica sem regra prévia. Retroativo (se habilitado) deve ser transacional. **Regras são tenant-scoped (`tenantId`; `NULL` = global).** Lookup é tenant-primeiro-depois-global; promoção a global é ADMIN-only (`POST /merchant-categories/promote-global`). TODOS os leitores (`credit-card`, `bank-account`, `agent-tools`, `pendencia`) e escritores (`setManual`/`removeManual`/`classifyBatch`) passam o `tenantId` — regra de um tenant nunca vaza para outro, e `removeManual` nunca apaga a global. Escrita global usa delete+create (não `upsert`: `NULL` em unique composto no SQLite não casa em `findUnique`).
...
22. **Teste com data FIXA + tela que filtra pelo MÊS CORRENTE = bomba-relógio, e o CI em UTC estoura antes do Brasil.** Em 2026-07-31 o CI ficou vermelho sem ninguém ter mudado código: em UTC já era 1º de agosto, os fixtures eram de julho/2026, e as telas filtram por `new Date()` → lista vazia → asserção falha. O sintoma engana duas vezes: (a) *parece* culpa do PR que estava aberto (o #384 levou a culpa, era inocente); (b) reproduzir localmente **passa**, porque no Brasil ainda era julho — só `TZ=UTC npx vitest run` / `CI=1 TZ=UTC npx playwright test` revela. Atingiu 1 unitário (`AvulsasTab.test.tsx`) e 7 e2e (`expenses-mobile.spec.ts`, `phase-d-responsive.spec.ts`). **A correção parcial é a armadilha real:** o primeiro conserto (PR #385) tratou só o unitário porque o levantamento olhou `Test Web`/`Test API`/`Test Domain` e não o job **E2E Web** — `Test Web` ficou verde e o PR *parecia* pronto com 7 testes ainda vermelhos. Ao escrever fixture com data, **congele o relógio**: `vi.setSystemTime()` (vitest, com `vi.useRealTimers()` no `afterEach`) ou `page.clock.setFixedTime()` **antes do `page.goto`** (Playwright ≥1.45). E valide a suíte **inteira** com `TZ=UTC`, os quatro jobs — não só o que falhou.

23. **CI verde + diff revisado NÃO é entrega verificada — dois PRs individualmente corretos quebram na soma, e quem paga é o PO abrindo o app.** Em 2026-08-01 mergeei quatro PRs no mesmo dia, todos com os sete jobs verdes e diff lido linha a linha. Em minutos de uso real o PO achou três regressões no onboarding: **botões duplicados** (`ImportMassStep.tsx` — o bloco `{noFunding && …}` já renderiza "Fatura do cartão"/"Extrato da conta" e dois blocos abaixo renderizam os mesmos sob a mesma condição, então `noFunding && cards.length > 0` mostra o botão **duas vezes**), um botão rotulado `"PDF, CSV, OFX, TXT"` (lista de formatos usada como rótulo de ação) e uma **colisão de rótulo** (o #391 renomeou o modo `foto` para "Fatura / Extrato", certo no menu "+", mas `ONBOARDING_MODES` reusa esses rótulos no painel da jornada, **onde já existe um botão "Importar"**). Nenhuma das três é alcançável por teste unitário do próprio componente: a duplicação exige a combinação `noFunding && cards.length > 0` que o teste não cobre, e a colisão só existe **em relação a outro componente da mesma tela**. Cada PR foi verificado isolado; a combinação — que é o que o usuário usa — não foi. **A regra: antes de dizer "pronto" e de novo depois de mergear um lote, alguém dirige a jornada real no app rodando** (conta nova criada pelo cadastro real, banco isolado com gate `lsof`, 375/390px e desktop, medição em runtime e **contagem** de botões repetidos — duplicata é invisível para humano rolando a tela e óbvia para `filter((t,i)=>arr.indexOf(t)!==i)`). Ordem de merge com dependência semântica conta: o #391 prometia XLS/XLSX no subtítulo e quem entrega o `accept` é o #392 — mergear C antes de A abriria janela em que o menu promete um formato que o seletor recusa. **Achou regressão no que já está mergeado: reverta primeiro** (`git revert <sha-do-squash>`), o revert é diff pequeno e seguro e o fix-forward não é, e o PO está usando a tela quebrada enquanto você escreve. A disciplina de QA em frota está em `.claude/agents/fleet-po.md` (seção "The QA ensemble").
