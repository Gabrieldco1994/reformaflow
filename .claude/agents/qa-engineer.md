...
- **No live DB in unit tests.** Services are tested against a **stateful mock-prisma harness** (see `conciliacao.service.spec.ts` `buildRateioPrisma`, `expense.service.spec.ts` PrismaMock). When you add a model interaction, extend the mock's interface AND impl or the suite won't compile. **A mock never runs the real `$use` middleware — it cannot prove anything about soft-delete visibility, only about which Prisma calls were made with which args.** Any behavior that depends on the middleware (whether a query sees a soft-deleted row, whether a `delete` becomes an `update`) is a claim the mock cannot settle; verify it against the real `PrismaService` (e.g. via a script that does `new PrismaService()` against a **disposable copy** of `prisma/dev.db`, or an e2e that imports `PrismaService`, not a bare `new PrismaClient()` — see `AGENTS.md` Scars, 2026-08-25, and the follow-up issue about e2e tests that construct a raw `PrismaClient` and never exercise the middleware they claim to test against). The old claim "`$transaction` bypasses `$use`" was false: `$use` intercepts by **action** (`findMany`/`findFirst`/`delete`/`deleteMany`), identically inside and outside a tx; `findUnique` is the one action never intercepted, tx or not.
