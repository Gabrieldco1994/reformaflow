...
- **Soft-delete middleware (`prisma.service.ts` `$use`).** `delete`→`update {deletedAt}`. Before
  ...
  write.
- **`$use` runs inside `$transaction` too — it is never bypassed by being in a tx.** Verified by execution (2026-08-25): `delete()` inside a tx still becomes a soft-delete `update` (the row survives with `deletedAt` set), and `findFirst`/`findMany` inside a tx still get `deletedAt: null` injected when you don't specify it. What actually varies is the **action**: the middleware only intercepts `findMany`, `findFirst`, `delete`, `deleteMany` — `findUnique` is never intercepted, in or out of a tx, and returns soft-deleted rows as-is. To see a soft-deleted row via `findFirst`/`findMany`, pass `deletedAt` explicitly in `where`; to see one by a unique key, use `findUnique`. A false version of this rule ("`$transaction` bypasses `$use`") lived in four normative files and produced a merged fix (#616) that didn't fix anything — see `AGENTS.md` Scars, 2026-08-25. For hard cleanup inside a tx use `deleteMany({where})` and let FK cascade do the rest (e.g. `FloorPlanRoom.reanalyze`) — that one works because `FloorPlanRoom` is in `modelsWithoutSoftDelete`, not because of any tx exemption.
