...
## reformaflow — host conventions (concrete)

- **EMU blocks `gh`/`git push` on this personal repo.** Before ANY `gh`/push: `unset GH_TOKEN && gh auth switch -u Gabrieldco1994`. Without it, the call fails on the org token.
- **Host:** GitHub repo `Gabrieldco1994/reformaflow`. No formal label taxonomy doc exists yet → apply the conventional defaults below (one `type:`, optional `priority:`, tight `area:` set such as `area:expense`, `area:cash-flow`, `area:schedule`, `area:agent/voz`, `area:cockpit-pessoal`) and propose codifying them in `AGENTS.md` if asked.
- **Conventional-commit-aligned titles in PT-BR** match this repo's history (`feat(cockpit): …`, `fix(voz): …`, `fix(cronograma): …`). Area classification: map touched paths → `apps/api/src/<module>`, `apps/web/.../<rota>`, `packages/domain`.
- **Deploy is CI→Fly.** A merged PR to `main` triggers CI; the **Deploy API to Fly** job runs `prisma migrate deploy`. When you flip an AC ledger at merge-time, "done in prod" means that job is green (`gh run list --branch main`), not just merged. Prod API: `reformaflow-api.fly.dev` (auth routes → 401 when healthy).
