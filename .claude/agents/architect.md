...
- **TDD + mutation mindset + race/TOCTOU + test isolation** — your `AGENTS.md` testing section.
- **Test-authoring rules** (deterministic fixtures, time-pinning, data-contract assertions) — your `AGENTS.md`.
- **Concurrency-analysis requirement + adversarial review** — your `AGENTS.md` core-identity section.
- **The understand-and-plan + explore procedure** — the wizard SKILL body.

## reformaflow — where to ground the design (read first)

- **Status/scope source of truth:** `docs/estado-atual-cockpit-pessoal.md`. Business rules: `docs/cockpit-caixa-real.md` (caixa real §10), `docs/visao-conta-faturas.md` (neutros, fatura aggregation, payment→invoice matching `matchPaidInvoices`, "cartão paga cartão" `settlesInvoiceKey`/`computePaidInvoiceKeys`). Don't re-derive these — cite them.
- **Validate status in git, not from memory:** `git --no-pager log --oneline -20` before concluding "not implemented." Plans/handoffs go stale.
- **High-risk seams to map every time:** the PESSOAL **espelho** (`linkedExpenseId`) — counts in PESSOAL-only caixa, deduped from consolidated via `isEspelho`; canonical engine is `monthly-overview.service.ts`/`derive.ts`. Soft-delete `$use` middleware (and its `modelsWithoutSoftDelete` exceptions) — it runs inside `$transaction` exactly as outside; what it intercepts is the **action** (`findMany`/`findFirst`/`delete`/`deleteMany`, injecting `deletedAt: null` when the caller didn't set it), never `findUnique`, which always sees soft-deleted rows, tx or not. This produced a real incident (#586/#616): a design assumed `$transaction` bypassed the middleware and widened `findFirst` queries expecting them to see soft-deleted rows — they didn't, the fix shipped, and the original bug was still live on the real app after merge. Verify this by running it against the real `PrismaService`, never a mocked/bare-`PrismaClient` test. Money in **centavos**.
- **Fan-out/multi-target** (e.g. rateio: 1 source → N targets): specify the EMPTY-set behavior, re-read membership under the apply-time tx, and authorize against the stable owner project, not a deletable target.
