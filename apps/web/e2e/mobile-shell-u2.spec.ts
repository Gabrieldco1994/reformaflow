...
async function mockApi(page: Page, baseURL: string, opts: MockOptions) {
  ...
  await page
    .context()
    .addCookies([{ name: "rf_token", value: "u2", url: baseURL }]);
  await page.context().route("http://localhost:3001/**", async (route) => {
    ...
    if (path === "/projects") {
      return route.fulfill(
        json([{ id: projectId, name: projectName, type: projectType }]),
      );
    }
...
async function expectReachable(
  target: Locator,
  name: string,
  scrollerSel: string | null = null,
) {
  await expect(
    target,
    `${name} não está visível — seletor congelado ausente?`,
  ).toBeVisible();
  ...
  const m = await target.evaluate((element, sel) => {
    ...
    return {
      ...
      topmost: topmost
        ? `${topmost.tagName.toLowerCase()}.${(topmost.className ?? "").toString().slice(0, 50)}`
        : "none",
      insideClip:
        cx >= clip.left &&
        cx <= clip.right &&
  ...
  expect(
    m.height,
    `${name} sem altura de toque (h=${m.height})`,
  ).toBeGreaterThanOrEqual(MIN_TOUCH);
  expect(
    m.width,
    `${name} sem largura de toque (w=${m.width})`,
  ).toBeGreaterThanOrEqual(MIN_TOUCH);
  expect(
    m.hitsSelf,
    `${name} não recebe o clique — coberto por ${m.topmost}`,
  ).toBe(true);
  expect(
    m.insideClip,
    `${name} centro em y=${m.cy} fora do recorte ${m.clip} (#507)`,
  ).toBe(true);
}
...
async function openMais(page: Page): Promise<Locator> {
  ...
  await expect(
    trigger,
    "gatilho Mais [data-mais-count] ausente — Lane A ainda não marcou",
  ).toBeVisible();
  await trigger.click();
  const overlay = page.locator('[data-overlay="mais"]');
  await expect(
    overlay,
    'overlay [data-overlay="mais"] não abriu',
  ).toBeVisible();
  return overlay;
}
...
test.describe("U2 shell mobile — provas comportamentais", () => {
  ...
  test("375 — U2-E20 usuário de PESSOAL com bankAccounts abre o próprio card sem cair em /no-permission", async ({
    ...
    await bootMobile(page, baseURL!, {
      modules: ["bankAccounts", "monthlyOverview"],
    });
    await page.goto("/projects");
    const card = page
      .locator(".md\\:hidden")
      .getByText("Pessoal U2", { exact: false })
      .first();
    ...
    await expect(page).toHaveURL(
      new RegExp(`/projects/${PESSOAL_ID}/monthly$`),
    );
  });
...
test.describe("U2 shell mobile — travas de regressão (verdes hoje)", () => {
  ...
  test("375 — [TRAVA] U2-E19 card do hub do usuário completo cai em /monthly", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
  ...
  test("375 — [TRAVA] U2-E17 deep-link /dre renderiza sem cair em /no-permission", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    ...
    expect(box!.height, "/dre <main> com caixa de altura zero").toBeGreaterThan(
      0,
    );
  });

  // U2-E14 — paisagem: o rail desktop assume e o dock some (>=768px ⇒ md:hidden).
  test("812×375 — [TRAVA] U2-E14 paisagem cai no rail desktop, dock ausente", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(
...
test.describe("U2 shell mobile — geometria do dock e do Mais", () => {
  for (const vp of VIEWPORTS) {
    ...
    test(`${vp.width} — U2-E01 dock: invariantes monthly·conta·maria + âncora + launcher alcançáveis`, async ({
      page,
      baseURL,
    }) => {
      ...
      await expect(
        dock,
        "dock [data-dock] ausente — Lane A ainda não marcou o DOM",
      ).toBeVisible();
      for (const slug of ["monthly", "conta", "maria"]) {
        await expectReachable(
          dock.locator(`[data-dock-slot="${slug}"]`),
          `slot invariante ${slug}`,
        );
      }
      ...
      await expectReachable(
        page.locator('[data-mobile-header] a[data-nav-group="projetos"]'),
        "âncora Projetos (cabeçalho)",
      );
      await expectReachable(
        page.locator('[data-launcher="true"]').first(),
        "launcher do dock",
      );
  ...
  test("375 — U2-E02 nenhum destino do dock é coberto (elementFromPoint devolve o próprio)", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    ...
    const covered = await dock
      .locator('[data-dock-slot], [data-launcher="true"]')
      .evaluateAll((els) =>
        els
          .map((el) => {
            const r = el.getBoundingClientRect();
    ...
    expect(
      covered,
      `destino(s) coberto(s): ${JSON.stringify(covered)}`,
    ).toEqual([]);
  });

  // U2-E03 — rótulos do dock ≥11px, uma linha, não elipsados (D7 — text-[10px]).
  for (const vp of VIEWPORTS) {
    test(`${vp.width} — U2-E03 rótulos do dock ≥11px, 1 linha, sem elipse`, async ({
      page,
      baseURL,
    }) => {
      await bootMobile(page, baseURL!, { modules: MODULES.full }, vp);
      ...
      const dock = page.locator("[data-dock]");
      await expect(dock, "dock [data-dock] ausente").toBeVisible();
      const bad = await dock
        .locator("[data-dock-slot]")
        .evaluateAll((slots) => {
          const out: unknown[] = [];
  ...
  test("375 — U2-E04 Mais: tiles ≥44×44, rótulo ≥11px, último alcançável após rolar", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    ...
    const bad = await tiles.evaluateAll((els) => {
      ...
      for (const el of els) {
        ...
        if (leaf && parseFloat(getComputedStyle(leaf).fontSize) < 11) {
          out.push({
            href: el.getAttribute("href"),
            fs: getComputedStyle(leaf).fontSize,
          });
        }
      }
      return out;
  ...
  for (const vp of VIEWPORTS) {
    test(`${vp.width} — U2-E11 sem estouro horizontal no cabeçalho, dock e Mais`, async ({
      page,
      baseURL,
    }) => {
      await bootMobile(page, baseURL!, { modules: MODULES.full }, vp);
      ...
      const findings = await page.evaluate((width) => {
        ...
        for (const region of document.querySelectorAll(
          "[data-mobile-header], [data-dock], [data-overlay]",
        )) {
  ...
  for (const height of [500, 400]) {
    test(`375×${height} — U2-E12 dock inteiro dentro da viewport`, async ({
      page,
      baseURL,
    }) => {
      await bootMobile(
  ...
  // era coberto).
  test("375 — U2-E13 reduced motion: sem transição/animação no dock, overlay e cabeçalho", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    ...
    const moving = await page.evaluate(() => {
      ...
      for (const region of document.querySelectorAll(
        "[data-dock], [data-overlay], [data-mobile-header]",
      )) {
        for (const el of [region, ...region.querySelectorAll("*")]) {
          const cs = getComputedStyle(el as HTMLElement);
          if (has(cs.transitionDuration) || has(cs.animationDuration)) {
            out.push({
              tag: el.tagName.toLowerCase(),
              t: cs.transitionDuration,
              a: cs.animationDuration,
            });
          }
        }
      }
    ...
    expect(
      moving,
      `elemento(s) ainda animando: ${JSON.stringify(moving)}`,
    ).toEqual([]);
  });
});
...
test.describe("U2 shell mobile — partição e permissão", () => {
  ...
  test("375 — U2-E05 dock ∪ Mais === visibleNav ∪ {maria}, sem duplicata", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    ...
    const dock = page.locator("[data-dock]");
    await expect(dock, "dock [data-dock] ausente").toBeVisible();
    const dockSlugs = await dock
      .locator("[data-dock-slot]")
      .evaluateAll((els) =>
        els.map((e) => e.getAttribute("data-dock-slot") ?? ""),
      );
    const overlay = await openMais(page);
    const maisRaw = await overlay
      .locator("a[href]")
      .evaluateAll((els) => els.map((e) => e.getAttribute("href") ?? ""));
    const maisSlugs = maisRaw
      .map(slugOf)
      .filter((s) => !SHELL_DEST_SLUGS.includes(s));
    ...
    expect(new Set(union), "partição ≠ visibleNav ∪ {maria}").toEqual(
      new Set([...EXPECTED_V.full, "maria"]),
    );
  });
  ...
  test("375 — U2-E16 visibleNav vazio: dock presente com só a Maria (correção PO)", async ({
    page,
    baseURL,
  }) => {
    ...
    await expect(
      dock,
      "dock deve PERMANECER — Maria fica mesmo com V vazio",
    ).toBeVisible();
    const slugs = await dock
      .locator("[data-dock-slot]")
      .evaluateAll((els) =>
        els.map((e) => e.getAttribute("data-dock-slot") ?? ""),
      );
  ...
  test("375 — U2-E21 sem creditCards: dock sem slot de Cartões, com monthly/conta", async ({
    page,
    baseURL,
  }) => {
    ...
    await expect(dock, "dock [data-dock] ausente").toBeVisible();
    await expect(
      dock.locator('[data-dock-slot="credit-cards"]'),
      "Cartões não deve estar no dock sem o módulo (D2)",
    ).toHaveCount(0);
...
test.describe("U2 shell mobile — cabeçalho / escopo", () => {
  ...
  test('375 — U2-E18 cabeçalho: data-scope-project-type=PESSOAL e âncora "Projetos"', async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    ...
    await expect(
      scope,
      "[data-scope-project-type] ausente — Lane A ainda não marcou",
    ).toHaveAttribute("data-scope-project-type", "PESSOAL");
    const anchor = page.locator(
      '[data-mobile-header] a[data-nav-group="projetos"]',
    );
    await expect(
      anchor,
      "âncora Projetos ausente no cabeçalho mobile",
    ).toBeVisible();
    await expect(
      anchor,
      'âncora deve ser "Projetos", não "Voltar para projetos"',
    ).toHaveAttribute("aria-label", "Projetos");
  });
});
...
test.describe("U2 shell mobile — overlay único e foco", () => {
  ...
  test("375 — U2-E06 abrir launch com o Mais aberto = um único overlay [data-overlay]", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    ...
    await expect(
      page.locator('[data-overlay="mais"]'),
      "Mais não cedeu ao launch (enum D4 furou — dois overlays)",
    ).toBeHidden();
    await expect(
      page.locator("[data-overlay]:visible"),
      "D4: esperado exatamente 1 overlay",
    ).toHaveCount(1);
  });

  // U2-E07 [D4] — launch aberto (deep-link), abrir o Mais por TECLADO não empilha.
  // Mesmo enum, gatilho a11y. Mede por [data-overlay].
  test("375 — U2-E07 launch aberto + Mais por teclado = um único overlay [data-overlay]", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    ...
    await expect(
      trigger,
      "gatilho Mais [data-mais-count] ausente",
    ).toBeVisible();
    await trigger.focus();
    await page.keyboard.press("Enter");
    await expect(
      page.locator('[data-overlay="mais"]'),
      "Mais não abriu por teclado",
    ).toBeVisible();
    await expect(
      launch,
      "launch não cedeu ao Mais (enum D4 furou — dois overlays)",
    ).toBeHidden();
    await expect(
      page.locator("[data-overlay]:visible"),
      "D4: esperado exatamente 1 overlay",
  ...
  test("375 — U2-E08 Escape fecha o Mais e devolve o foco ao gatilho", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
  ...
  test("375 — U2-E09 back fecha o overlay e mantém a rota", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    ...
    await expect(
      page,
      "back saiu da rota em vez de fechar o overlay",
    ).toHaveURL(new RegExp(`${PESSOAL_ID}/monthly`));
    await expect(
      page.locator('[data-overlay="mais"]'),
      "overlay ainda aberto após back",
    ).toBeHidden();
  });
});
...
test.describe("U2 shell mobile — preservação de mês", () => {
  ...
  test("375 — U2-P14 mês sobrevive ao toque no dock (monthly → conta)", async ({
    page,
    baseURL,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== "mobile",
      "375 mobile contract runs only in mobile project; desktop coverage lives elsewhere",
    );
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    // Readiness: load the exact destination in a throwaway page and wait for
    // its month-specific content before the real dock click.
    const warmup = await page.context().newPage();
    await warmup.goto(`/projects/${PESSOAL_ID}/conta?mes=${TEST_MONTH}`);
    await expect(
      warmup.locator("main"),
      "warm-up conta not ready",
    ).toContainText(TEST_MONTH_LABEL);
    await warmup.close();
    await page.goto(`/projects/${PESSOAL_ID}/monthly?mes=${TEST_MONTH}`);
    const conta = page.locator('[data-dock-slot="conta"]');
    await expect(
      conta,
      "slot conta ausente no dock — Lane A ainda não marcou",
    ).toBeVisible();
    await conta.click();
    await expect(page, "dock dropou o mês (href sem ?mes)").toHaveURL(
      new RegExp(`/conta\\?.*mes=${TEST_MONTH}`),
    );
    await expect(
      page.locator("main"),
      "/conta ignorou ?mes (mostra o mês corrente)",
    ).toContainText(TEST_MONTH_LABEL);
  ...
  test("375 — U2-P15 shell carrega o mês ao Mais (monthly → dre), URL-only", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    ...
    await expect(
      tile,
      "tile dre ausente no Mais — Lane A ainda não marcou",
    ).toBeVisible();
    await tile.click();
    // contrato do shell (minha lane): o mês viaja na URL (mes OU period). A
    // LEITURA pelo destino é fatia própria — ver o DIAG P21 `dre`.
    await expect(page, "Mais dropou o mês (href sem mês)").toHaveURL(
      /\/dre\?.*(mes|period)=2026-03/,
    );
  });
  ...
  test("1280 — U2-P16 mês sobrevive no rail desktop (monthly → conta)", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(
    ...
    await expect(page, "rail dropou o mês (href sem ?mes)").toHaveURL(
      new RegExp(`/conta\\?.*mes=${TEST_MONTH}`),
    );
    await expect(page.locator("main"), "/conta ignorou ?mes").toContainText(
      TEST_MONTH_LABEL,
    );
  });
  ...
  test("375 — U2-P17 launch não reabre ao tocar o dock, mas o mês sobrevive", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    await page.goto(
      `/projects/${PESSOAL_ID}/monthly?launch=1&mes=${TEST_MONTH}`,
    );
    const launch = page.locator('[data-overlay="launch"]');
    await expect(
      launch,
      "launch overlay ausente — Lane A ainda não marcou",
    ).toBeVisible();
    await page.keyboard.press("Escape");
    ...
    await conta.click();
    await expect(
      page.locator('[data-overlay="launch"]'),
      "launch reabriu (passthrough proibido)",
    ).toHaveCount(0);
    await expect(page, "mês não sobreviveu ao dock").toHaveURL(
      new RegExp(`/conta\\?.*mes=${TEST_MONTH}`),
    );
  });
  ...
  test("375 — U2-P19 estado ativo aceso com ?mes na URL (dock)", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    ...
    await expect(
      slot,
      "estado ativo morreu com ?mes na URL (§3)",
    ).toHaveAttribute("data-active", "true");
    await expect(
      slot,
      "aria-current deve acender na página exata",
...
test.describe("U2 shell mobile — travas e diagnóstico (2ª prioridade)", () => {
  ...
  test("375 — [TRAVA] U2-E15 reduzido: nenhum href de módulo ausente no shell", async ({
    page,
    baseURL,
  }) => {
    ...
    await expect(
      page.locator("[data-mobile-header]"),
      "shell não renderizou — skip silencioso?",
    ).toBeVisible();
    const forbidden = [
      "monthly",
      "conta",
    ...
    const hrefs = await page.evaluate(() => {
      ...
      for (const root of roots)
        for (const a of root.querySelectorAll("a[href]"))
          acc.push(a.getAttribute("href") ?? "");
      return acc;
    });
    const leaked = [
      ...new Set(hrefs.map(slugOf).filter((s) => forbidden.includes(s))),
    ];
    expect(
      leaked,
      `href(s) de módulo sem permissão vazando no SHELL: ${leaked.join(",")}`,
    ).toEqual([]);
  });
  ...
  test("375 — U2-P18 focus não vaza entre destinos (href do tile dropa focus)", async ({
    page,
    baseURL,
  }) => {
    await bootMobile(page, baseURL!, { modules: MODULES.full });
    await page.goto(
      `/projects/${PESSOAL_ID}/monthly?mes=${TEST_MONTH}&focus=closingDay`,
    );
    const overlay = await openMais(page);
    ...
    await expect(
      tile,
      "tile cash-flow ausente no Mais — Lane A ainda não marcou",
    ).toBeVisible();
    ...
    expect(
      href,
      `focus vazou para o tile (não está no allowlist): ${href}`,
    ).not.toMatch(/focus=/);
  });
  ...
  const NAO_COBERTOS_POR_MES: Array<{
    slug: string;
    mesTeste: RegExp;
    mesCorrente?: RegExp;
  }> = [
  ...
  for (const { slug, mesTeste, mesCorrente } of NAO_COBERTOS_POR_MES) {
    test(`375 — [DIAG] U2-P21 ${slug} ignora ?mes (cobertura parcial declarada)`, async ({
      page,
      baseURL,
    }) => {
      await bootMobile(page, baseURL!, { modules: MODULES.full });
      await page.goto(`/projects/${PESSOAL_ID}/${slug}?mes=${TEST_MONTH}`);
      await expect(page, `${slug} caiu em /no-permission`).not.toHaveURL(
        /no-permission/,
      );
      ...
      if (mesCorrente) {
        ...
        expect(
          visivel,
          `${slug} não mostrou o mês CORRENTE (visível) — probe errado?`,
        ).toMatch(mesCorrente);
      }
