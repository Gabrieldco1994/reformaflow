...
async function mockApi(page: Page, baseURL: string) {
  ...
  await page
    .context()
    .addCookies([{ name: "rf_token", value: "imfr", url: baseURL }]);
  await page.route("http://localhost:3001/**", async (route) => {
    ...
    if (p === "/auth/config")
      return route.fulfill(j({ registerEnabled: false, guestEnabled: false }));
    if (p === "/projects")
      return route.fulfill(j([{ id: PID, name: "Pessoal", type: "PESSOAL" }]));
    if (p === `/projects/${PID}`)
      return route.fulfill(
        j({
          id: PID,
    ...
    if (/\/bank-accounts$/.test(p) || p === "/tenant/bank-accounts")
      return route.fulfill(
        j([
          {
            id: "imfr-a1",
          ...
          },
          {
            ...
            projectId: PID,
            institution: "BB",
            nickname: "Conta IMFR Dois",
            last4: "2222",
          },
        ]),
      );
    if (/\/credit-cards$/.test(p) || p === "/tenant/credit-cards")
      return route.fulfill(
        j([
          {
            id: "imfr-c1",
    ...
    if (/^\/projects\/[^/]+\/expenses$/.test(p))
      return route.fulfill(
        j({ items: [], total: 0, page: 1, pageSize: 2000, totalPages: 0 }),
      );
    if (/^\/projects\/[^/]+\/expenses\/paid-origins$/.test(p))
      return route.fulfill(j({ items: [] }));
    if (p.endsWith("/monthly-overview/account-view"))
      return route.fulfill(j(ACCOUNT_VIEW));
    if (p.endsWith("/monthly-overview/dre-overview"))
      return route.fulfill(j({ anual: { saldoAcumuladoSerie: [] } }));
    if (p.endsWith("/monthly-overview/origin-items-yearly"))
      return route.fulfill(
        j({ year: 2026, kind: "all", last4: "", total: 0, items: [] }),
      );
    if (p === "/journeys/eligible") return route.fulfill(j([]));
    if (p.includes("/bank-accounts/") && p.endsWith("/import-statement"))
      return route.fulfill(
        j(
    ...
    if (p.includes("/credit-cards/") && p.endsWith("/import-statement"))
      return route.fulfill(
        j(
          url.searchParams.get("mode") === "commit"
            ? {
...
async function openImportModal(
  ...
  if (kind === "extrato") {
    ...
    await page
      ...
      .getByRole("button", { name: /Conta IMFR.*1111/ })
      .dispatchEvent("click");
  } else {
...
async function focusIsOnLauncher(page: Page) {
  return page.evaluate(() => {
    ...
    if (!el || el.tagName === "BODY")
      return { ok: false, where: el?.tagName?.toLowerCase() ?? "null" };
    if (!document.body.contains(el)) return { ok: false, where: "detached" };
    if (el.closest('[data-mobile-sheet="modal"]'))
      return { ok: false, where: "ainda no modal" };
    const isLauncher =
      el.getAttribute("data-launcher") === "true" ||
      (el.tagName === "BUTTON" &&
        (el.getAttribute("aria-label") === "Lançar" ||
          /^\s*Lançar\s*$/.test(el.textContent || "")));
    return {
      ok: isLauncher,
      where: isLauncher
        ? "Lançar"
...
for (const kind of ["extrato", "fatura"] as const) {
  for (const via of ["Escape", "Cancelar", "Fechar", "Concluir"] as const) {
    test(`${kind} · fechar por ${via} devolve o foco ao "Lançar"`, async ({
      page,
      baseURL,
    }, ti) => {
      ...
      if (via === "Escape") {
        ...
        await dialog
          .getByRole("button", { name: /Confirmar importação/i })
          .click();
        await dialog
      ...
      await expect
        .poll(async () => (await focusIsOnLauncher(page)).ok, {
          timeout: 2000,
          message: `${kind}/${via}: foco não voltou ao "Lançar"`,
        })
        .toBe(true);
    });
  }
