...
const BANK_PREVIEW = {
  ...
  totalAmountCents: 12000, totalDebits: 2, totalCredits: 0,
  preview: [
    { externalId: 'ictt-nova', date: '2026-09-01', merchant: 'Padaria', amountCents: 7000, category: null, duplicate: false, willImport: true },
    { externalId: 'ictt-dup', date: '2026-06-10', merchant: 'Mercado', amountCents: 5000, category: null, duplicate: false, willImport: false, possibleDuplicate: POSSIBLE_DUP },
  ],
  possibleDuplicates: [POSSIBLE_DUP],
};

const CARD_PREVIEW = {
  source: 'CSV_NUBANK', periodLabel: '2026-09', total: 2, duplicated: 0, totalAmountCents: 12000,
  preview: [
    { externalId: 'ictt-nova', date: '2026-09-01', merchant: 'Padaria', amountCents: 7000, category: null, installmentCurrent: null, installmentTotal: null, duplicate: false, willImport: true },
    { externalId: 'ictt-dup', date: '2026-06-10', merchant: 'Mercado', amountCents: 5000, category: null, installmentCurrent: null, installmentTotal: null, duplicate: false, willImport: false, possibleDuplicate: POSSIBLE_DUP },
  ],
  possibleDuplicates: [POSSIBLE_DUP],
};

async function mockApi(page: Page, baseURL: string) {
  await page.context().addCookies([{ name: 'rf_token', value: 'ictt-test', url: baseURL }]);
  await page.route('http://localhost:3001/**', async (route) => {
    const url = new URL(route.request().url());
    const path = url.pathname;
    if (path === '/auth/me') {
      return route.fulfill(json({
        id: 'ictt-user', username: 'ictt-test', name: 'ICTT', role: 'USER', isGuest: false,
        tenantId: 'ictt-tenant', allowedModules: [...ALL_MODULES],
    ...
    if (/\/bank-accounts$/.test(path) || path === '/tenant/bank-accounts') {
      ...
      return route.fulfill(json([
        { id: ACCOUNT_ID, projectId: PESSOAL_ID, institution: 'ITAU', nickname: 'Conta ICTT', last4: '1111' },
        { id: 'ictt-acc-2', projectId: PESSOAL_ID, institution: 'BB', nickname: 'Conta ICTT Dois', last4: '2222' },
      ]));
    }
    if (/\/credit-cards$/.test(path) || path === '/tenant/credit-cards') {
      return route.fulfill(json([
        ...
        { id: 'ictt-card-2', projectId: PESSOAL_ID, institution: 'ITAU', brand: 'Visa', nickname: 'Azul', last4: '5555' },
      ]));
    }
...
async function openImportModal(page: Page, kind: 'extrato' | 'fatura', mobile: boolean) {
  ...
  if (kind === 'extrato') {
    ...
    await picker.getByRole('button', { name: /Conta ICTT.*1111/ }).dispatchEvent('click');
  } else {
...
async function noHorizontalOverflow(page: Page) {
  return page.evaluate(() => {
    ...
    return de.scrollWidth <= de.clientWidth + 1 && document.body.scrollWidth <= de.clientWidth + 1;
  });
}

for (const kind of ['extrato', 'fatura'] as const) {
  test(`${kind}: "Confirmar importação" >= 44px e clicável (viewport do projeto)`, async ({ page, baseURL }, testInfo) => {
    const mobile = isMobile(testInfo);
    await mockApi(page, baseURL!);
    const dialog = await openImportModal(page, kind, mobile);

    const confirmar = dialog.getByRole('button', { name: /Confirmar importação/i });
    ...
    const centerHit = await page.evaluate(({ x, y }) => {
      const btn = [...document.querySelectorAll('button')].find((b) => /Confirmar importação/i.test(b.textContent ?? ''));
  ...
  test(`${kind}: "Confirmar importação" >= 44px em 375px`, async ({ page, baseURL }, testInfo) => {
...
test('extrato: rótulo "Importar mesmo assim" da linha Tier B >= 44px e clicável', async ({ page, baseURL }, testInfo) => {
  const mobile = isMobile(testInfo);
  await mockApi(page, baseURL!);
  ...
  const hit = await page.evaluate(({ x, y }) => {
    const lab = [...document.querySelectorAll('label')].find((l) => /Importar mesmo assim/.test(l.textContent ?? ''));
  ...
  await expect(dialog.locator('p', { hasText: 'Mesma data e valor' })).toContainText('10/06/2026');
});
