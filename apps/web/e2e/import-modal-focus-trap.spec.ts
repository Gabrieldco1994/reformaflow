...
async function mockApi(page: Page, baseURL: string) {
  ...
  await page
    .context()
    .addCookies([{ name: "rf_token", value: "imft-test", url: baseURL }]);
...
async function openImportModal(
  ...
  if (kind === "extrato") {
    ...
    await picker
      .getByRole("button", { name: /Conta IMFT.*1111/ })
      .dispatchEvent("click");
  } else {
...
test.describe("Import Modal — Focus Trap (#680)", () => {
  ...
  test("extrato: Shift+Tab reiterado não sai do modal (375px)", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    const dialog = await openImportModal(page, "extrato", true);
    ...
    for (let i = 0; i < 15; i++) {
      ...
      expect(isInModal, `Shift+Tab #${i + 1} não deve sair do modal`).toBe(
        true,
      );
    }
  });
  ...
  test("fatura: Shift+Tab reiterado não sai do modal (375px)", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    const dialog = await openImportModal(page, "fatura", true);
  ...
  test('modal tem role="dialog" e aria-modal="true"', async ({
    page,
  }, testInfo) => {
