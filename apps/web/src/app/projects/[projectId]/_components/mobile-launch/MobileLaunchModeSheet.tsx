        ...
        {view === 'root' ? (
          <div className="space-y-2.5 pt-1">
            {MOBILE_LAUNCH_MODES.map((mode) => {
              ...
              // Despesa mode: render with journey action marker for mobile
              if (mode.value === 'despesa') {
                return (
                  <button
                    key={mode.value}
                    type="button"
          ...
          <div className="space-y-2.5 pt-1">
            {PHOTO_MODES.map((mode) => {
              ...
              // First photo mode (fatura) is the primary import entry point
              if (mode.value === 'fatura') {
                return (
                  <button
                    key={mode.value}
                    type="button"
                    data-journey-action="import.start"
                    onClick={() => onPick(mode.value)}
                    className="flex min-h-[68px] w-full items-center gap-3.5 rounded-2xl border border-darc-linen bg-lifeone-surface px-4 py-3 text-left transition-transform active:scale-[0.99]"
                  >
                    <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${accentMap[mode.value]}`}>
