...
import { Card, Progress, type Tone } from './ui';
...
function usageTone(percent: number): Tone {
  if (percent >= 100) return 'neg';
  if (percent >= 80) return 'alert';
  return 'pos';
}
          ...
          <section className="space-y-2">
            ...
            <div className="space-y-2">
              {visibleCards.map((card) => {
                ...
                return (
                  <div key={card.id} className="rounded-xl border border-[var(--ck-border)] bg-[var(--ck-surface-2)]/55 px-3 py-2">
                    ...
                    {canShowUsage ? (
                      <>
                        <Progress value={pct / 100} tone={usageTone(pct)} />
                        <div className="mt-1 flex justify-between gap-2 text-[10px] text-[var(--ck-muted)]">
                          <span>{pct}% usado</span>
                          {availableCents != null && <span>disp. {fmtMoneyExact(availableCents)}</span>}
                        </div>
                      </>
                    ) : availableCents != null ? (
                      <p className="text-[10px] text-[var(--ck-muted)]">disponível {fmtMoneyExact(availableCents)}</p>
                    ) : null}
                  </div>
                );
              })}
