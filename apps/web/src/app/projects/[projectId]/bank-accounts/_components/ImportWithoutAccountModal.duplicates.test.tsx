...
async function toPreview() {
  render(<ImportWithoutAccountModal projectId="p1" onClose={vi.fn()} onCommitted={vi.fn()} />);
  fireEvent.change(screen.getByLabelText('Arquivos'), { target: { files: [file()] } });
...
describe('ImportWithoutAccountModal — Tier B possível duplicata (#659)', () => {
  it('mostra o aviso + contagem no resumo; commit padrão não força a linha', async () => {
    upload().mockResolvedValueOnce(PREVIEW).mockResolvedValueOnce({ inserted: 1, failed: 0 });
    await toPreview();

    expect(screen.getByText(/Conferência:/)).toHaveTextContent('1 possível(is) duplicata(s)');
    ...
    expect(
      decisionsFromLastUpload().find((d) => d.externalId === 'e2' && d.action === 'import'),
    ).toBeUndefined();
  });

  it('opt-in explícito → commit manda action:"import" para a linha', async () => {
    upload().mockResolvedValueOnce(PREVIEW).mockResolvedValueOnce({ inserted: 2, failed: 0 });
    await toPreview();
  ...
  it('trocar o tipo de documento limpa o opt-in', async () => {
    ...
    await toPreview();
    await userEvent.click(screen.getByRole('checkbox', { name: /importar mesmo assim/i }));
    expect(screen.getByRole('checkbox', { name: /importar mesmo assim/i })).toBeChecked();

    fireEvent.click(screen.getByRole('button', { name: 'Fatura de cartão' }));
    // prévia descartada — refaz e o opt-in volta desmarcado
    fireEvent.click(screen.getByRole('button', { name: 'Conferir arquivos' }));
    await screen.findByText(/Conferência:/);
    expect(screen.getByRole('checkbox', { name: /importar mesmo assim/i })).not.toBeChecked();
  });
});
