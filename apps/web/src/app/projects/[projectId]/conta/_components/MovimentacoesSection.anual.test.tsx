...
const MESES = Array.from({ length: 12 }, (_, index) => String(index + 1).padStart(2, '0'));
...
function anoResponse(): AccountViewYearlyResponse {
  ...
  return {
    ...
    saidas: meses.flatMap((m) => m.saidas).sort((a, b) => b.data.localeCompare(a.data)),
    comprasCartao: [],
    entradas: meses.flatMap((m) => m.entradas).sort((a, b) => b.data.localeCompare(a.data)),
    ticketMedio: { valor: 0, nCompras: 0, totalCompras: 0, serie12m: [], mediaAnual: 0, deltaVsMediaPct: null },
  };
}
...
function gruposRenderizados() {
  return screen
    ...
    .map((el) => el.getAttribute('aria-label')!.replace('Movimentações de ', ''));
}

function linhasRenderizadas() {
  return screen
    .getAllByRole('region', { name: /^Movimentações de / })
    .flatMap((group) => within(group).getAllByText(/^(Mercado|Feira|Salário) \d{2}$/))
...
describe('MovimentacoesSection — visão anual', () => {
  ...
  it('fala do ANO no cabeçalho e no filtro rápido — nunca "no mês"', () => {
    renderSection(anoResponse(), { mode: 'ano', summaryQuickFilter: 'entrouMes' });

    expect(screen.getByText('Movimentações do ano')).toBeInTheDocument();
    expect(screen.getByText(/Filtro rápido: Entrou no ano/)).toBeInTheDocument();
