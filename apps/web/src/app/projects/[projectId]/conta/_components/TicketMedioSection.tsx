...
function deltaTone(value: number | null) {
  ...
  return value > 0 ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-700';
}
...
export function TicketMedioSection({
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm xl:p-6">
      <div className="xl:grid xl:grid-cols-[18rem_minmax(0,1fr)] xl:gap-8">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">
            Ticket médio
          </p>
          <h2 className="mt-1 text-lg font-bold text-slate-950">Ticket médio do mês</h2>
          <p className="mt-2 text-2xl font-bold tracking-tight text-slate-950 xl:text-[34px]">
            ...
            {ticket.serie6m.map((item) => {
              ...
              return (
                <div key={item.mes} className="flex flex-1 flex-col items-center gap-2">
                  ...
                  <span className={`text-[11px] font-semibold ${active ? 'text-slate-950' : 'text-slate-500'}`}>
