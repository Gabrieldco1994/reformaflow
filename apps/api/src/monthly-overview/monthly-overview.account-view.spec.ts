...
describe("MonthlyOverviewService.getAccountView", () => {
  ...
  it("monta a visão da conta sem dobrar pagamento de fatura neutro", async () => {
    ...
    expect(
      res.saidas.some((item: any) => /Pagamento fatura/i.test(item.descricao)),
    ).toBe(false);
    expect(
      res.saidas.reduce((sum: number, item: any) => sum + item.valor, 0),
    ).toBe(20_000);

    const comprasNubank = res.comprasCartao.filter(
      (item: any) => item.cardLast4 === "1111",
    );
    expect(comprasNubank.length).toBeGreaterThan(0);
    expect(comprasNubank.every((item: any) => item.isInvoice === false)).toBe(
      true,
    );
    expect(
      comprasNubank.reduce((sum: number, item: any) => sum + item.valor, 0),
    ).toBe(7_000);
    ...
    expect(res.entradas).toEqual([
      expect.objectContaining({
        descricao: "Reembolso",
        // Descrição do usuário presente → descricaoRaw espelha o texto (sem label).
        descricaoRaw: "Reembolso",
        tipo: "reembolso",
        valor: 4_000,
        status: "PREVISTO",
      }),
  ...
  it("entrada sem descrição cai no rótulo do tipo e mantém descricaoRaw nula (fallback do map)", async () => {
    // Blinda o branch de fallback do map de entradas:
    //   descricao: receipt.descricao?.trim() || receiptTypeLabel(receipt.tipo)
    //   descricaoRaw: receipt.descricao?.trim() || null
    // Sem este teste, trocar o fallback por `receipt.descricao` puro sobreviveria
    // (o teste feliz acima usa receipts com descrição preenchida).
    ...
    prisma.receipt.findMany.mockResolvedValue([
      ...
      },
      {
        ...
        tipo: "REEMBOLSO",
        // Só espaços → trim vazio → mesmo fallback, descricaoRaw nula.
        descricao: "   ",
        status: "PREVISTO",
        bankLast4: "4247",
      },
    ...
    const salario = res.entradas.find((e: any) => e.id === "rec-null-desc");
    expect(salario).toMatchObject({
      descricao: "Salario", // receiptTypeLabel('SALARIO')
      descricaoRaw: null,
      status: "EM_CAIXA",
      valor: 3_000,
    });

    const reembolso = res.entradas.find((e: any) => e.id === "rec-blank-desc");
    expect(reembolso).toMatchObject({
      descricao: "Reembolso", // receiptTypeLabel('REEMBOLSO')
      descricaoRaw: null,
      status: "PREVISTO",
      valor: 1_000,
    });
  ...
  it("inclui cobrança neutra no cartão (PgConta/Pix no crédito) na fatura, mas fora do gasto real", async () => {
    ...
    const compras9999 = res.comprasCartao.filter(
      (item: any) => item.cardLast4 === "9999",
    );
    expect(
      compras9999.reduce((sum: number, item: any) => sum + item.valor, 0),
    ).toBe(1_000);
    expect(
      compras9999.some((item: any) => /PgConta/i.test(item.descricao)),
    ).toBe(false);
  ...
  it("getCardInvoicesYearly agrupa faturas por mês e inclui conta corrente", async () => {
    ...
    const res: any = await service.getCardInvoicesYearly(
      tenantId,
      projectId,
      2026,
    );
    ...
    expect(res.origins.map((o: any) => o.key)).toEqual([
      "card:9999",
      "conta:4247",
    ]);
    expect(res.origins.find((o: any) => o.kind === "conta").nickname).toBe(
      "Itau",
    );
  ...
  it("casa pagamento da fatura pelo valor: vencimento dia 1 pago no mês anterior marca a fatura certa", async () => {
    ...
    prisma.cashFlowEntry.findMany.mockResolvedValue([
      entryFor(purchaseJun),
      entryFor(purchaseJul),
    ]);
    prisma.creditCard.findMany.mockResolvedValue([
      {
        id: "card-nu",
    ...
    const jul: any = await service.getAccountView(
      tenantId,
      projectId,
      "2026-07",
    );
    ...
    const jun: any = await service.getAccountView(
      tenantId,
      projectId,
      "2026-06",
    );
  ...
  it("cartão paga cartão: vínculo explícito quita a fatura do cartão pago sem inflar caixa", async () => {
    ...
    const latamJun: any = await service.getAccountView(
      tenantId,
      projectId,
      "2026-06",
    );
    ...
    const nubankMai: any = await service.getAccountView(
      tenantId,
      projectId,
      "2026-05",
    );
  ...
  it("separa contas com mesmo last4 usando importId e prioriza Itaú como conta principal do caixa", async () => {
    ...
    prisma.bankStatementImport.findMany.mockResolvedValue([
      { id: "imp-nu", accountId: "acc-nubank" },
    ]);
    prisma.expense.findMany.mockResolvedValue([
      {
        id: "exp-itau",
  ...
  describe("getAccountView — parcelas foreign bank-paid (espelho na conta)", () => {
    ...
    it("emite as parcelas FUTURAS não pagas de uma foreign QUINZENAL bank-paid no mês do vencimento", async () => {
      prisma.expense.findMany.mockResolvedValue([
        base({
          ...
          status: "PAGO",
          bankLast4: "3636",
          dataPagamento: new Date("2026-05-24T00:00:00.000Z"),
          linkedExpenseId: "foreign-q",
        }),
        base({
          id: "esp-1",
          projectId,
        ...
        base({
          ...
          titulo: "Infra Eletrica",
          fornecedor: "RMD ENG",
          valorTotal: 12_000,
          valor: 3_000,
          formaPagamento: "QUINZENAL",
          quantidadeParcela: 4,
          dataInicioParcela: new Date("2026-05-24T00:00:00.000Z"),
          paidParcelas: "[0,1]",
          status: "PLANEJADO",
          project: reforma,
        }),
      ...
      const emitida = res.saidas.find(
        (s: any) =>
          s.descricao === "Infra Eletrica" && s.status === "PLANEJADO",
      );
    ...
    it("NÃO emite saída de conta para foreign PARCELADO paga por CARTÃO (evita dupla contagem)", async () => {
      ...
      prisma.expense.findMany.mockResolvedValue([
        base({
          ...
          linkedExpenseId: "foreign-p",
        }),
        base({
          id: "foreign-p",
          projectId: reforma.id,
          titulo: "Calhas e Rufos",
          fornecedor: "Calhas",
          valorTotal: 8_000,
          valor: 4_000,
          formaPagamento: "PARCELADO",
      ...
      const contaReforma = res.saidas.filter(
        (s: any) =>
          s.isInvoice === false && s.projetoOrigem?.type === "REFORMA",
      );
    ...
    it("foreign PARCELADO coberta por CARTÃO com settlement só na 1ª parcela NÃO vaza as futuras (regressão dupla contagem cross-project)", async () => {
      ...
      prisma.crossProjectSettlement.findMany.mockResolvedValue([
        {
          tenantId,
          targetExpenseId: "foreign-set",
          parcelaIndex: 0,
          sourceExpenseId: "esp-full",
          realValor: 1_000,
          plannedValor: 1_000,
      ...
      expect(
        res.saidas.some((s: any) => String(s.id).startsWith("foreign-set#")),
      ).toBe(false);
      const contaReforma = res.saidas.filter(
        (s: any) =>
          s.isInvoice === false && s.projetoOrigem?.type === "REFORMA",
    ...
    it("mantém excluída a foreign À VISTA quitada por espelho bank (não re-emite)", async () => {
      prisma.expense.findMany.mockResolvedValue([
        base({
          ...
          bankLast4: "3636",
          dataPagamento: new Date("2026-06-05T00:00:00.000Z"),
          linkedExpenseId: "foreign-av",
        }),
        base({
          id: "foreign-av",
          projectId: reforma.id,
          titulo: "Material à vista",
          fornecedor: "Loja",
          valorTotal: 5_000,
          valor: 5_000,
          formaPagamento: "A_VISTA",
    ...
    it("preserva o lump (valorTotal) para foreign SEM espelho (não linkada)", async () => {
      ...
      const lump = res.saidas.find(
        (s: any) => s.descricao === "Material avulso",
      );
      expect(lump).toBeDefined();
      expect(lump.valor).toBe(7_000);
      expect(lump.bankLast4).toBeNull();
      expect(lump.projetoOrigem).toEqual(
        expect.objectContaining({ type: "REFORMA" }),
      );
...
describe("matchPaidInvoices", () => {
  ...
  it("casa o pagamento pela proximidade de valor dentro da janela {payMonth, payMonth+1}", () => {
    ...
    const invoices = [
      inv("2026-06", "3541", 442_034),
      inv("2026-07", "3541", 2_401_031),
    ];
    const payments = [
      pay("2026-05", "3541", 442_034),
      pay("2026-06", "3541", 2_401_033),
    ];
  ...
  it("preserva o casamento no mesmo mês quando o pagamento ocorre no mês do vencimento", () => {
    const invoices = [
      inv("2026-06", "1111", 7_000),
      inv("2026-07", "1111", 2_000),
    ];
  ...
  it("isola por cartão e ignora pagamento sem fatura na janela", () => {
    ...
    const payments = [
      pay("2026-06", "BBBB", 1_000),
      pay("2026-01", "AAAA", 1_000),
    ];
    const paid = matchPaidInvoices(invoices, payments);
  ...
  it("consome cada fatura uma única vez (dois pagamentos, duas faturas distintas)", () => {
    const invoices = [
      inv("2026-06", "9999", 5_000),
      inv("2026-07", "9999", 5_000),
    ];
    const payments = [
      pay("2026-05", "9999", 5_000),
      pay("2026-06", "9999", 5_000),
    ];
    const paid = matchPaidInvoices(invoices, payments);
...
describe("computePaidInvoiceKeys", () => {
  ...
  it("quita com dois pagamentos parciais no mesmo ciclo", () => {
    ...
    const implicit = [
      pay("2026-06", "9999", 50_000),
      pay("2026-06", "9999", 190_000),
    ];
    const paid = computePaidInvoiceKeys(invoices, implicit, []);
    expect(paid.has("2026-07__9999")).toBe(true);
  });
...
describe("MonthlyOverviewService.getAccountView — Carteira (origem='none')", () => {
  ...
  it("T9: sobraPrevista mensal segue a matriz conta/carteira sem dupla contagem", async () => {
    const scenarios = [
      {
        id: "paga-com-conta",
        expense: base({
        ...
        expected: { caixaHoje: 94_500, carteiraHoje: 0, saiuMes: 5_500, faltaPagarMes: 0, sobraPrevista: 94_500 },
      },
      {
        id: "paga-sem-conta",
        expense: base({
        ...
        expected: { caixaHoje: 100_000, carteiraHoje: -5_500, saiuMes: 5_500, faltaPagarMes: 0, sobraPrevista: 94_500 },
      },
      {
        ...
        expected: { caixaHoje: 100_000, carteiraHoje: 0, saiuMes: 0, faltaPagarMes: 5_500, sobraPrevista: 94_500 },
      },
      {
    ...
    for (const scenario of scenarios) {
      prisma.expense.findMany.mockResolvedValueOnce([scenario.expense]);
      const res: any = await service.getAccountView(tenantId, projectId, "2026-06");

      expect(res.caixaHoje).toBe(scenario.expected.caixaHoje);
      expect(res.carteiraHoje).toBe(scenario.expected.carteiraHoje);
      expect(res.saiuMes).toBe(scenario.expected.saiuMes);
      expect(res.faltaPagarMes).toBe(scenario.expected.faltaPagarMes);
      expect(res.sobraPrevista).toBe(scenario.expected.sobraPrevista);
    }
  });
});
