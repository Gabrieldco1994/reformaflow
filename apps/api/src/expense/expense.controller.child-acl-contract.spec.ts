...
function buildController(serviceOverrides: Record<string, jest.Mock>) {
  const service = {
    ...
    updateInstallmentDate: jest.fn().mockResolvedValue({ ok: true }),
    ...serviceOverrides,
  };
...
describe("ExpenseController — repassa requester ao service nas mutações child-ACL (#448 B1a)", () => {
  ...
  it("conciliarParcela: requester chega ao service", async () => {
    ...
    await (controller as any).conciliarParcela(
      ...
      "p1",
      "src-1",
      { targetExpenseId: "tgt-1", parcelaIndex: 0, realValor: 100 },
      REQUESTER,
    );
  ...
  it("ratear: requester chega ao service", async () => {
    ...
    await (controller as any).ratear(
      ...
      "p1",
      "src-1",
      { allocations: [{ targetExpenseId: "tgt-1", allocation: 100 }] },
      REQUESTER,
    );
  ...
  it("ratearMixed: o REQUESTER INTEIRO (não só requester.id) chega ao service", async () => {
    const { controller, service } = buildController({});
    await controller.ratearMixed("t1", "p1", "src-1", REQUESTER, {
      newTargets: [],
      existing: [],
    });
  ...
  it.each([
    ...
    "%s repassa o requester completo às reversões",
    async (method, args) => {
      const { controller, service } = buildController({});
      await (
        controller[method] as (...callArgs: unknown[]) => Promise<unknown>
      )(...args);
  ...
  it("updateInstallmentDate: requester completo chega ao service como último argumento", async () => {
    const { controller, service } = buildController({});
    await (controller as any).updateInstallmentDate(
      "t1",
      "p1",
      "expense-1",
      { parcela: 1, data: "2026-09-20" },
      REQUESTER,
    );

    expect(service.updateInstallmentDate).toHaveBeenCalledWith(
      "t1",
      "p1",
      "expense-1",
      1,
