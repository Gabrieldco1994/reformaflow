...
export class ExpenseService {
  ...
  async linkAccount(
    ...
    const expense = await this.prisma.expense.findFirst({
      where: { id: expenseId, projectId, tenantId, deletedAt: null },
    });
    if (!expense) throw new NotFoundException('Despesa não encontrada');
    ...
    if (
      ...
      expense.bankLast4 === (account.last4 ?? null) &&
  ...
  async linkCard(
    ...
    const card = await this.prisma.creditCard.findFirst({
      where: { id: cardId, projectId, tenantId, deletedAt: null },
      select: { last4: true },
    });
    if (!card) throw new NotFoundException('Cartão de crédito não encontrado');

    if (
      expense.cardLast4 === (card.last4 ?? null) &&
      expense.accountId === null &&
      expense.bankLast4 === null &&
      expense.origin === 'import'
    ) {
      return expense;
    }
