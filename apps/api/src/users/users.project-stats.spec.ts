...
describe('UsersService.getProjectStats', () => {
  ...
  it('returns response with correct shape', async () => {
    ...
    expect(stats).toHaveProperty('contentTodayByType');
    expect(stats).toHaveProperty('contentTodayTotal');
    expect(stats).toHaveProperty('windowStart');
    expect(stats).toHaveProperty('windowEnd');

    expect(Array.isArray(stats.byType)).toBe(true);
    expect(Array.isArray(stats.contentTodayByType)).toBe(true);
    expect(typeof stats.contentTodayTotal).toBe('number');
    expect(typeof stats.windowStart).toBe('string');
    expect(typeof stats.windowEnd).toBe('string');
  });
});
