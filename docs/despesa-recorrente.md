...
13. **Escopo do `obraProjectId` (B1a — Security Phase 2):** o requester é obrigatório e o `obraProjectId` é validado contra o tenant **e** o escopo autorizado do requisitante via `canRequesterSeeProject` — antes de qualquer verificação de tipo ou escrita. Requester ausente ou projeto fora do escopo → `BadRequestException('Projeto de obra não encontrado neste tenant.')`, a mesma exceção já lançada para projeto inexistente ou cross-tenant; nunca um `403` que confirmaria existência ou tipo do projeto ao requisitante.
